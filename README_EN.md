...
The repository ships a Codex-native, repository-scoped skill setup. On Windows:
...
cd workspaces\cumcm
codex
# In Codex: $1start-mathmodel solve this modeling problem
```

This keeps skills and generated competition work inside the cloned repository. Run `scripts/setup-local.ps1` without `-CodexOnly` to install the optional legacy WebUI locally as well.
...
- 🧮 Python plus MATLAB/Octave workflows, with an optional official MathWorks MCP and independent batch reproduction
- 📄 17 Typst plus 17 matching LaTeX competition templates
- 📝 Generate a well-formatted paper
- 🤝 Auditable multi-agent orchestration with sealed context packets, disagreement records, and human-only gates
- 🔄 Optional heterogeneous OpenAI-compatible reviewers, including a fail-closed DeepSeek adapter
- 🌐 Spatial audits for 2D/3D coordinates, distance, topology, trajectories, and coverage
- 🧪 Structure-aware algorithm search, PSO baseline, bounded parallel runs, and cross-seed statistics
...
- [x] LaTeX template integration (17 counterparts for the Typst families)
...
- [x] MATLAB in the Codex skills workflow (generation, CLI execution, artifacts, appendix, reproducibility checks)
- [ ] R language
