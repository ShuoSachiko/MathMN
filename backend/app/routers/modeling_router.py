...
async def exampleModeling(
    ...
    # 示例数据根目录；source 参数通过 pathlib resolve 后做前缀校验，
    # 防止用户用 ../ 路径逃逸出 examples 目录读取任意文件
    examples_root = Path("app/example/example").resolve()
    example_dir = (examples_root / example_request.source).resolve()
    if examples_root not in example_dir.parents:
        raise HTTPException(status_code=400, detail="非法的示例名称")
    ic(example_dir)
    ...
    for file in current_files:
        src_file = example_dir / file
        dst_file = Path(work_dir) / file
