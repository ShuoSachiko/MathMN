...
class LLM:
    ...
    async def chat(
        ...
        """调用 LLM 完成一次对话，失败时按指数退避自动重试。

        Args:
            history: 对话历史（OpenAI 格式）。
            tools: 可选工具定义列表。
            tool_choice: 工具选择策略。
            max_retries: 重试上限；为 None 时回退到 settings.LLM_MAX_RETRIES。
            retry_delay: 首次退避的基准秒数。
            top_p: 采样参数。
            agent_name: 调用方 Agent 名称，用于消息路由。
            sub_title: 子任务标题。
        ...
        # 重试上限优先取调用方显式传入值，否则回退到全局配置，
        # 避免网络故障时无限重试挂死任务
        retry_limit = max_retries if max_retries is not None else settings.LLM_MAX_RETRIES
        ...
        while True:
            try:
                ...
                if attempt >= retry_limit:
                    raise
                # 指数退避：base * 2^(attempt-1)，上限 30 秒；
                # 使用 asyncio.sleep 而非 time.sleep，避免阻塞事件循环
                backoff = min(retry_delay * (2 ** (attempt - 1)), 30.0)
