...
class WriterAgent(Agent):
    ...
    async def run(  # type: ignore[reportIncompatibleMethodOverride]
        ...
        if self.scholar is None:
            # 未配置 OpenAlex 邮箱时不下发文献检索工具，让写作手直接撰写而非崩溃
            tools = None
        else:
        ...
        if response.tool_calls:
            ...
            if tool_call.name == "search_papers":
                ...
                # scholar 为 None 时本不会下发该工具，这里兜底避免意外崩溃
                if self.scholar is None:
                    logger.warning("未配置 OpenAlex 邮箱，跳过文献检索")
                    return WriterResponse(
                        response_content=response.content or "", footnotes=footnotes
                    )
                try:
