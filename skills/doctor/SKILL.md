---
name: doctor
description: "数学建模环境检查与安装向导。用于检查 Codex、Typst/LaTeX、Python、MATLAB/Octave、绘图和 PDF 工具；结合 plan.md 判断必需项，并为 Windows/macOS/Linux 提供安装建议。手动触发。"
---
...
| `matlab` | MATLAB 建模代码执行（选择 MATLAB 时优先） | `command -v matlab` |
| `octave` | MATLAB 兼容代码的开源备用运行时 | `command -v octave` |
...
check_cmd matlab || check_cmd octave   # 选择 MATLAB 时至少一个
...
**必须项：** 所选论文引擎、Python（工作流脚本需要）；选择 Python 编程时需要 numpy/pandas/matplotlib，选择 MATLAB 编程时需要 MATLAB 或 Octave
...
| 平台 | 命令或方式 |
| --- | --- |
| Windows/macOS/Linux | MATLAB 使用 MathWorks 安装器和有效许可证；安装后确认 `matlab -batch "disp(version)"` 可运行 |
| Windows | `winget install GNU.Octave`（仅作为兼容运行时） |
| macOS | `brew install --cask octave-app` |
| Linux (apt) | `sudo apt install octave` |
...
Windows PowerShell 可运行 `skills/doctor/scripts/doctor.ps1` 获得包含 Codex、MATLAB 和项目本地依赖的 JSON/表格报告。
...
```bash
python "<3coding-visual skill>/scripts/matlab_agentic_status.py"
```

MATLAB 本体是运行 MATLAB 任务的必须项；Agentic Toolkit 是增强项，不得把它误报成 MATLAB 本体。缺少 toolkit 时给出仓库脚本 `scripts/setup-matlab-agentic.ps1` 和 MathWorks 官方安装来源；只有用户明确运行 `-Install` 时才下载到 `.runtime/` 并注册 Codex，不把 GNU Octave 宣称为支持该 MCP。
