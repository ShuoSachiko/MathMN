...
Options:
  ...
  --problem-analysis FILE
                          Analysis file for structural context. Defaults to <root-dir>/reports/ANALYSIS_MODELING_REPORT.md.
...
if [ -z "$PROBLEM_ANALYSIS_FILE" ] && [ -f "$ROOT_DIR/reports/ANALYSIS_MODELING_REPORT.md" ]; then
  PROBLEM_ANALYSIS_FILE="$ROOT_DIR/reports/ANALYSIS_MODELING_REPORT.md"
fi
...
if includes:
    ...
    fail(f"main has no include/input calls")
...
if problem_analysis and problem_analysis.exists():
    info(
        "analysis file loaded for text context; ReqID coverage is checked by "
        "integrity_check.py, not inferred from section filenames"
    )
else:
    info("problem analysis file not supplied/found")

placeholder_re = re.compile(r"PLACEHOLDER|TODO|TBD|XXX|待补充|待续写|这里补|示例数据|待完善|示例数值|待替换|EXAMPLE-VALUE|示例代码|【占位|PLACEHOLDER:|占位：|use supported claim")
...
if results_file and results_file.exists():
    info(
        "results file loaded; key-by-key numeric and unit consistency is checked "
        "by integrity_check.py against claim_ledger.json"
    )
else:
    info("results file not supplied/found")

if all_results and all_results.exists():
    try:
        data = json.loads(read(all_results))
        info(
            "legacy all-results JSON parses successfully; substring matching is "
            "intentionally not used as a numeric-consistency proof"
        )
    except Exception as exc:
        fail(f"cannot parse explicitly supplied all-results JSON: {exc}")
else:
