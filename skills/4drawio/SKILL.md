---
name: 4drawio
description: "数学建模非数据图示阶段。仅在能澄清冻结模型和 supported claims 时，根据题意合同、分析/结果报告和已有 figures/ 生成技术路线、模型结构或数据处理等 DrawIO 图，并导出可追溯 PDF。"
---
...
读取 `../_references/modeling_integrity.md` 的题意合同和结论证据要求；如需领域判断，再读取 `../_references/math_modeling_norms.md` 中的“图表与可视化”和“非数据图工具选择”小节。
...
如果不需要非数据图，只在 `reports/DRAWIO_REPORT.md` 说明即可；不得规定最低图数，也不得为了显得完整强制生成技术路线图。
...
先确认 analysis 与 coding gate 为 PASS 且版本哈希当前。读取 `PROBLEM_CONTRACT.json`、`claim_ledger.json`、`reports/ANALYSIS_MODELING_REPORT.md`、`reports/RESULTS_REPORT.md` 和 `figures/` 目录列表。图中只能表达合同和 supported claim 已存在的关系，不能发明流程、因果或结果。
...
[ ] fig_flow_<ReqID> 某个确有解释价值的要求求解流程图
...
每张图在 `DRAWIO_REPORT.md` 记录关联 ReqID/claim ID、源文档版本和选择理由。人类对不同表达方案反复权衡时保存分任务候选并 diff/select，不覆盖旧图；版本选择变化会使论文引用变为 STALE。
